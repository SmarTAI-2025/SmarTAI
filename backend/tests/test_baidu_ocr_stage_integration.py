...
async def test_question_ocr_uncertain_submit_is_not_repeated(monkeypatch):
    ...
    async def submit(stored_file_id: str | None = None):
        return await task_preparation.preflight_problem_source(
            task_id=task_id,
            file=(
                None
    ...
    stored_file_id = first.value.detail["stored_file_id"]
    with pytest.raises(HTTPException) as replay:
        await submit(stored_file_id)
    assert replay.value.detail["code"] == "provider_submit_uncertain"
    assert replay.value.detail["stored_file_id"] == stored_file_id
