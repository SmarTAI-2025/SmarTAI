...
def test_current_task_uses_owner_selected_provider(history_client, monkeypatch):
    client, registry_default = history_client
    selected = Provider({"filters": {"needs_attention": True}, "sort": "attention_first"})
    owners = []
    ...
    monkeypatch.setattr(tasks, "resolve_owner_default_provider", resolve)
    response = client.post("/tasks/query/interpret", json={"query": "先把那些需要我接手的摆到前面"})
    assert response.status_code == 200, response.text
    assert owners == ["history-owner"]
