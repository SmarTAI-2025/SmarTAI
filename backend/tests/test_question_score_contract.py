...
async def test_selected_text_model_preserves_question_image_for_model_switch(
    ...
    registry = MagicMock()
    registry.pick_default_id.return_value = "text-provider"
    registry.pick_default.return_value = provider
    registry.pick_vision.return_value = None
    registry.get.return_value = provider
    registry.list_configs.return_value = [{
        "provider_id": "text-provider",
    ...
    with pytest.raises(HTTPException) as exc:
        await preflight_problem_source(
            ...
            extraction_hint="",
            save_to_library=False,
            recognition_provider_id="text-provider",
            current=SimpleNamespace(id=owner_id),
            registry=registry,
        )

    assert exc.value.status_code == 422
    stored_file_id = exc.value.detail.get("stored_file_id")
    ...
    assert stored.kind == "problem_source"
