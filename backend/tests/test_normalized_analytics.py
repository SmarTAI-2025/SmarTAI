...
def test_filter_intent_uses_owner_selected_provider(monkeypatch):
    owner = _user("teacher", "intent-selected-provider")
    seeded = _seed_graded_assignment(owner)
    registry_default = _Provider()
    selected = _Provider()
    owners = []

    def resolve(owner_id, registry):
        owners.append(owner_id)
        return selected

    monkeypatch.setattr(analytics, "resolve_owner_default_provider", resolve)
    response = _client(owner, _Registry(registry_default)).post(
        f"/analytics/{seeded['task_id']}/filter-intent",
        json={"question": "从高到低", "surface": "student_analysis"},
    )
    assert response.status_code == 200, response.text
...
@pytest.mark.parametrize("surface, fields, recognized", [
    ...
    ("student_answer_review", {
        "sort": "question_desc", "submission_status": "missing", "question_tokens": ["Q1"],
    }, True),
    ("student_answer_review", {"sort": "score_desc"}, False),
    ("student_analysis", {"sort": "id_desc"}, True),
    ("student_analysis", {"sort": "confidence_desc"}, True),
    ("review_overview", {"sort": "review_asc"}, True),
    ("question_analysis", {"sort": "coverage_desc"}, True),
