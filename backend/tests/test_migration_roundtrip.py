...
def test_source_outcome_migration_preserves_0003_rows(tmp_path, monkeypatch):
    ...
    command.downgrade(cfg, "0003_assignment_workflow_facade")
...
def test_postgresql_source_outcome_ddl_is_portable(monkeypatch):
    sql = _postgresql_sql(monkeypatch, "0004_workflow_source_outcomes")
...
def test_postgresql_source_outcome_downgrade_drops_child_first(monkeypatch):
    sql = _postgresql_sql(
        monkeypatch,
        "0004_workflow_source_outcomes:0003_assignment_workflow_facade",
    )
