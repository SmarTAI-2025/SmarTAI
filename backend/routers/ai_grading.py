...
from dependencies import OPENAI_API_KEY, OPENAI_API_BASE, OPENAI_MODEL
...
async def process_student_submission(student: Dict[str, Any], problem_store: Dict[str, Any]) -> Dict[str, Any]:
    ...
    student_id = student.get("stu_id", None)
    student_name = student.get("stu_name", "未识别姓名")
    if not student_id:
        return None
...
async def run_grading_task(job_id: str, student_id: str, problem_store: Dict, student_store: Dict[str, Any]):
    ...
    try:
        ...
        GRADING_RESULTS[job_id] = {
            "status": "completed",
            "student_id": student_id,
            "student_name": student_data.get("stu_name", "未识别姓名"),
