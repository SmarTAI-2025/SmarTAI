...
@router.post("/ask")
async def ask_teacher(req: AskRequest, current: User = Depends(require_task_teacher),
                      registry: ExpertRegistry = Depends(get_scoped_expert_registry)):
    return await _run_grounded_ask(None, req, current, registry)


@router.post("/{task_id}/ask")
async def ask_task(task_id: str, req: AskRequest, current: User = Depends(require_task_teacher),
                   registry: ExpertRegistry = Depends(get_scoped_expert_registry)):
    return await _run_grounded_ask(task_id, req, current, registry)


@router.post("/{task_id}/query")
async def nl_query(task_id: str, req: QueryRequest, current: User = Depends(require_task_teacher),
                   registry: ExpertRegistry = Depends(get_scoped_expert_registry)):
