...
1. **宣传页可以使用合成数据、前端动画和预计算 walkthrough；进入 Live Demo 后，API、OCR 和批改必须真实运行。**
   ...
   - 四份 Live 合成作答固定使用虚构姓名与编号：Alex Chen / DEMO-001、Maya Lin / DEMO-002、Jordan Rivera / DEMO-003、Taylor Singh / DEMO-004；姓名和编号同时写入文件名与原件页眉，由真实识别流程返回，前端不得硬填识别结果。排版与混排作答使用 PDF，手写与扫描作答使用原始 PNG 直接进入视觉 OCR，避免额外的图片 PDF 渲染层掩盖真实识别问题。
   - Demo 会话中的任务元数据、题目来源和学生作答全部固定为可见的合成样例：新建/编辑任务页只读展示示例名称与范围；上传题目页只显示 `question_source.pdf`、四题摘要、原文件入口和返回同一 `taskId` 的 Live CTA；上传学生作答页只显示四份具名预置文件、原件入口和同一 Live CTA。Demo UI 不允许改名或选择自有文件，普通教师任务保持完整编辑/上传流程。两个预置 CTA 仍由 Live 编排器通过真实 API 上传并继续题目准备或 OCR，不通过浏览器脚本伪造文件选择。该 UI 约束不改变当前 `frontier_demo` scope 仍可调用整个 task API 的已知简化边界，不能表述为后端上传白名单。
   ...
   - Live 批改完成后的即时分析只读取本次真实 API 返回的逐题结果；采用马卡龙色指标卡与自动轮播图集，至少覆盖学生总分走势、成绩区间直方图、逐题散点和教师复核信号环图。轮播必须可手动切换/暂停并尊重 reduced-motion，禁止把宣传页静态数字混入真实结果。
...
- **2026-08-11**：项目负责人确认四份 Live 合成作答需要稳定虚构姓名/编号并在文件名和原件页眉双重呈现；手写与扫描样本改用原始 PNG 直接进入真实视觉 OCR，排版与混排保留 PDF。Demo 会话中的任务信息、题目来源与学生作答固定为只读可见样例，不允许在产品页改名或选择自有文件；题目页与作答页都提供原文件入口和返回同一 Live 任务的明确 CTA，普通任务不变。批改完成后的真实结果区改为马卡龙色、多图型、可暂停且尊重 reduced-motion 的自动轮播，所有数值继续只来自本次 API 结果。该前端锁定不替代已知未实现的后端 fixture 白名单。
...
- **2026-08-11**：项目负责人确认任务完成 AI 批改后，即使教师尚未完成确认与发布，也应允许进入只读的结果总览、题目分析、学生分析和可视化分析；页面必须明确标注“未确认结果”。报告、下载、对外发布与正式结果版本仍由教师复核确认门禁控制。本决定替代“确认前完全锁定结果分析”的旧前端行为，不改变后端正式结果与制品生成门禁。
