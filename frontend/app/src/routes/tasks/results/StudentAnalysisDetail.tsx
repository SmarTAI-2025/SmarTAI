...
export function StudentAnalysisDetail({ locale, taskId, studentId, model }: { locale: Locale; taskId: string; studentId: string; model: ResultsModel }) {
  ...
  const { filter: questionFilter, visibleQuestions } = useResultQuestionFilter({ taskId, questions: model.questions, locale, studentId });
  ...
  return (
    <section className="rounded-[10px] border bg-card p-5">
      ...
      <ResultQuestionQuery className="mt-4" locale={locale} taskId={taskId} filter={questionFilter}
        questions={visibleQuestions} onSelect={scrollToQuestion} />
