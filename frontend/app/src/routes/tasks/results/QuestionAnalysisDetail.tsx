import { ResultQuestionSearch } from "@/components/tasks/ResultQuestionSearch";
...
export function QuestionAnalysisDetail({
  ...
  const { filter: questionFilter, matches, visibleQuestions } = useResultQuestionFilter({ taskId, query, questions: model.questions, locale });
  ...
  return (
    <section className="rounded-[10px] border bg-card p-5">
      ...
      <ResultQuestionSearch className="mt-4" locale={locale} value={query} matches={matches} onQuery={updateQuery} onSelect={goToQuestion} filter={questionFilter} taskId={taskId} />

      <div className="mt-2 grid gap-0.5 text-[10px] leading-4 text-muted-foreground">
        <p>{tx(locale, "本地预设优先；其他表达点击应用后由模型理解。", "Local presets run first; apply other requests for model interpretation.")}</p>
