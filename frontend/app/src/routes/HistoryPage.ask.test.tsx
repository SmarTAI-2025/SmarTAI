...
describe("Current task Ask SmarTAI", () => {
  ...
  it("discloses when deterministic history parsing did not call a model", async () => {
    mocks.interpret.mockResolvedValue({ ...interpretation, source: "deterministic" });
    mount();
    ask();

    expect(await screen.findByText("本地规则已识别，未调用模型。")).toBeInTheDocument();
  });
  ...
  it("toggles a visible table header between ascending and descending order", async () => {
    mount();

    expect(screen.queryByRole("combobox", { name: "排序" })).not.toBeInTheDocument();

    const taskHeader = screen.getByRole("button", { name: "任务。点击按升序排序。" });
    ...
    fireEvent.click(screen.getByRole("button", { name: "任务，当前升序。点击切换为降序。" }));
