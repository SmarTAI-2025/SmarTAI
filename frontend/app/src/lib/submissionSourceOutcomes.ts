...
const REASON_COPY: Record<string, ReasonFactory> = {
  ...
  provider_timeout: (locale) => providerCopy(locale, "模型响应超时", "The model timed out", "稍后重试；原文件已经保存，不需要重新整理整批文件。", "Retry later. The original is saved, so the whole batch does not need to be rebuilt."),
  provider_unreachable: (locale) => providerCopy(locale, "暂时无法连接模型服务", "The model service is unreachable", "检查网络或模型服务状态后重试。", "Check network and provider status, then retry."),
  provider_rate_limited: (locale) => providerCopy(locale, "模型服务触发限流", "The model service rate-limited the request", "等待限额恢复后重试，或换用另一个已启用模型。", "Wait for the quota window or retry with another enabled model."),
  provider_auth_failed: (locale) => providerCopy(locale, "模型凭据无效或无权限", "The model credentials were rejected", "到 BYOK 检查 API Key、模型权限和余额后重试。", "Check the API key, model permission, and balance in BYOK, then retry."),
  provider_credentials_unavailable: (locale) => providerCopy(locale, "无法读取已保存的模型凭据", "Saved model credentials could not be loaded", "重新保存 BYOK 凭据后重试。", "Save the BYOK credentials again, then retry."),
...
function providerCopy(locale: Locale, zhTitle: string, enTitle: string, zhStep: string, enStep: string): SubmissionSourceReasonCopy {
  return {
    title: tx(locale, zhTitle, enTitle),
    description: tx(locale, "原文件已保存，但本次模型调用没有得到可用结果。", "The original file is saved, but this model call produced no usable result."),
