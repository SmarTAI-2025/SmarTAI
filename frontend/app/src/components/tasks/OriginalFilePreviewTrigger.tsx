  ...
  if (unavailable) {
    return (
      <span className={cn("inline-flex min-w-0 items-center gap-1", className)}>
        <Button type="button" variant="secondary" className="h-10 px-3" disabled><Eye aria-hidden="true" className="h-4 w-4" />{resolvedOpenLabel}</Button>
        <HelpTooltip label={unavailableReason(descriptor.unavailable_reason, t)} />
      </span>
    );
  }
