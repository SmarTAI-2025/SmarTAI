...
### 品牌素材与使用层级（2026-08-12 已确认）

- 正式字标统一复用项目负责人提供的 `字标.svg` 轮廓；浅色页面使用主色约为 `#489FEC` 的蓝色版本，深色宣传表面只允许改材质/颜色为白色或银白，不得重画字形。
- Frontier 首屏可采用更具宣传感的蓝晶积分笔 App 商标并放大呈现；暗色最终 CTA 使用白银积分笔与白色字标，导航、产品窗口和页脚使用克制的正式蓝色字标。
- `SmarTAI` 的品牌释义为 `Smart AI Teaching Assistant`，只在宣传首屏主字标下以低权重英文说明出现一次；它不进入工作台全局导航，也不在各功能区重复展开。
- 圆角方框不是商标的强制组成部分：晶蓝/白银 App 图可以在宣传表面保留深色圆角底，扁平蓝色积分笔可在产品界面按空间决定是否加容器，但背景、商标颜色和对比度必须服从所在页面视觉体系。
- 旧的“蓝色圆角方块 + 普通白色 S”不是正式 App 商标，favicon、导航、页脚及其他明显品牌位必须删除或换成正式积分笔/字标，禁止继续新增。
- 本分支已完成 `/frontier`、`/frontier/enter`、`/frontier/live`、Demo 会话工作台壳与 favicon：宣传首屏使用蓝晶，Demo 暗底使用白银配白色字标，工作台壳使用克制蓝色字标。正式产品的扁平蓝实现保留在独立分支，避免把宣传/Demo 改动混入正式基线。
