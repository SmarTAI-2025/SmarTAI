...
- 批改配置 Setup。
- 个人知识库前端先行页 `/knowledge-base`，用于整理本地资料清单。
- 上传题目、题目预览与题干/评分标准编辑。
...
- 个人知识库只作为前端先行体验：侧边栏入口、资料清单、Setup 选择器、上传本任务资料时可勾选加入个人清单。
- 不声称已实现真正跨任务复用、云端保存或参与批改的用户级全局知识库。
...
- 会让用户以为 LMS 或用户级全局知识库后端已完成的页面、按钮、导航或空状态文案。
...
| task-scoped KB 上传/删除 | 已接入 `/tasks/{id}/kb` | 是 | 否 | 文案限定为本任务资料；真实参与当前后端 RAG |
| 结果区 KB 状态反馈 | 已接入任务资料状态提示 | 是 | 否 | 显示配置/资料状态，不声称逐题引用命中 |
| 用户级全局知识库前端 | 前端先行已接入 | 是 | 是 | `/knowledge-base` + Setup 选择器；当前只存浏览器本地元数据与选择关系 |
| 用户级全局知识库后端 | 未实现 | 否 | 是 | 真正跨任务复用、文件保存、检索和任务引用见 `BACKEND_TODOS_CN.md` |
...
- [x] 左侧栏个人知识库入口与前端先行页（完成：2026-07-01 约20:15；仅浏览器本地元数据）
- [x] Setup 中选择个人知识库资料的前端控件（完成：2026-07-01 约20:15；选择关系仅本地保存，当前不影响后端批改）
- [x] 上传本任务资料时可勾选加入个人知识库前端清单（完成：2026-07-01 约20:15；仍需后端 user-scoped KB API 才能真正跨任务复用）
...
- [x] 阶段性源码审计：task KB 文案未声称后端全局复用（完成：2026-07-01 约14:30）
...
- [x] 审计脚本更新为允许“个人知识库前端先行/后端待接入”文案，但仍拦截无免责声明的支持声明（完成：2026-07-01 约20:15）
