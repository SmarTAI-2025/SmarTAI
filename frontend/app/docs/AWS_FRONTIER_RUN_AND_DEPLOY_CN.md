...
### 0.10 少量用户 Demo 的并发设置

```dotenv
SMARTAI_MAX_CONCURRENT_LLM_PER_PROVIDER=5
```

新共享模型配置使用此变量限制**同一模型实例正在进行的 API 请求数**；不是在线用户数，也不是固定的题目／作答数量。题目提取可能一次处理多题，作答按文件解析，批改按学生和题目展开，最终均在模型调用处排队。填 `5` 保持此前实际默认并发；填 `1`／`2` 才降低并发。BYOK 仍使用其各自模型配置。

这是一次操作内的实例级限制，不是共享 Key 的平台总闸：两位用户各运行一次，合计可能达到 `2 × 5 = 10` 个模型请求。1～2位用户、小样例测试可先保持 `5`；若厂商额度较低或出现 429，降低到 `1`／`2`。并发不等于 RPM，低并发仍可能超过每分钟请求限额；已有 429 等待重试保留，本补丁不增加全平台 RPM 调度或自动提额。

OpenAI 兼容中转返回的 HTTP 5xx 会使用安全码 `provider_unreachable`，Live Demo 显示模型服务暂时不可用，并标记实际失败步骤。它不改变厂商502本身，也不改变 Responses 的流式／非流式方式。

### 0.11 Ask SmarTAI、Demo 会话与发布后页面恢复

更新前后端到同一 Demo 提交后，在 **Render 后端**新增或修改：

```dotenv
SMARTAI_HISTORY_QUERY_LLM_ENABLED=true
```

这会启用“当前任务”已有的自然语言理解模块；固定筛选仍可直接完成，未识别的表达才调用当前默认模型。沿用每位用户默认每日20次、间隔10秒的限制（`SMARTAI_HISTORY_QUERY_LLM_DAILY_LIMIT` / `SMARTAI_HISTORY_QUERY_LLM_COOLDOWN_SECONDS`）；触发限制或模型失败时会明确提示剩余文本按关键词搜索。关闭开关仍可使用本地筛选，但不会启用模型理解。

- 结果图表、题目／学生分析、复核筛选和当前任务的 Ask 模型请求，前端最长等待 **5分钟**，无需新增超时环境变量；上游主动失败或其自身超时仍会提前返回错误。
- 题目分析接入自然语言筛选，学生／复核支持姓名排序；不能支持的组合条件明确说明，不静默只执行其中一部分。图表使用真实置信度与复核统计，未知数据保持未知。
- Demo访问令牌仍默认20分钟；有效签名可在首次签发起 **2小时内**续期，保留相同用户、任务权限和共享额度。到达总时限需重新开始演示。正常登录的Cookie刷新保持原流程，无需清理Cookie，也不要更换JWT密钥。
- 前端发布后，请先手动刷新已经打开的旧页面一次，以获取新代码。新版遇到旧动态资源失效时，每标签页最多自动刷新一次并保留任务URL；持续失败显示可重载的错误页面。此问题与后端模型并发设置无关。

回归建议：保留同一演示任务，依次验证题目分析自然语言、学生姓名排序、复核筛选、自定义图表，以及“当前任务”的非预设表达。真实模型成功率和耗时需要部署后自行验证；本地模拟测试不代表中转或厂商稳定性。
