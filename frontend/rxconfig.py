...
_backend_only = os.environ.get("SMARTAI_REFLEX_BACKEND_ONLY", "").lower() in {
    "1",
    "true",
    "yes",
}
...
config = rx.Config(
    ...
    # In split production deployment, Render runs the Reflex state service with
    # `--backend-only`. The CLI rejects any frontend_port in that mode, so the
    # state service sets SMARTAI_REFLEX_BACKEND_ONLY=true to disable it here.
    frontend_port=(
        None
        if _backend_only
        else int(os.environ.get("REFLEX_FRONTEND_PORT", os.environ.get("PORT", 3000)))
    ),
