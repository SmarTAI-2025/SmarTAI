...
- [ ] Add migration `0005_workflow_operation_checkpoints`, using batch alter for SQLite compatibility, server defaults for existing rows, and a downgrade that removes only the new columns.
...
- [ ] Run `python -m alembic heads`, expecting only `0005_workflow_operation_checkpoints`.
