...
- Create: `backend/db/migrations/versions/0014_password_reset_requests.py`
...
Expected: FAIL because revision `0014_password_reset_requests` and the new columns/table do not exist.

- [ ] **Step 3: Implement the ORM and Alembic migration**

Add `UserRecord.auth_invalid_before: float | None`. Add `PasswordResetRequestRecord` with a foreign key to `users`, SHA-256 digest length, `created_at`, `expires_at`, `resend_available_at`, `superseded_at`, `consumed_at`, `delivery_status`, `last_delivery_error_code`, and `source_ip`. Create migration `0014` with `down_revision = "0013_email_verification_requests"`, indexes on user/email/time/token digest, and a nullable `users.auth_invalid_before` column.
...
```bash
git add backend/db/models.py backend/db/migrations/versions/0014_password_reset_requests.py backend/tests/test_migration_roundtrip.py
