...
题目准备与批改共用“模型与 BYOK”中每条模型配置的“并发上限”和 RPM，不再使用
单独的大题生成并发环境变量。不同账号、模型或中转地址可以分别设置；题目准备会为
每道需要补齐资料的计分大题发起一次模型请求，大题内部的 `(a)/(b)` 等小问仍属于
同一次请求，不会增加并发计数。

后端仍保留同一 endpoint 的进程级安全上限，避免多条模型配置同时把同一个服务压满：

```dotenv
SMARTAI_MAX_CONCURRENT_LLM_PER_ENDPOINT=5
```

一次题目准备任务的有效并发取“所选 BYOK 并发上限”和上述 endpoint 安全上限中的
较小值；RPM 继续按该 BYOK 配置排队。通常只需在“模型与 BYOK”页面调整对应模型，
部署环境变量只用于服务器整体保护。
